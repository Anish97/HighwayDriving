...
int main() {
  ...
  h.onMessage([&left_clear,&right_clear,&ref_vel,&lane,&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          bool too_close = false;
			bool left_clear_check = true;
          	bool right_clear_check = true;
          ...
          for(int i =0;i<sensor_fusion.size();i++){
            ...
            if((d>4*lane) && (d<4*(lane+1))){
              ...
              if((check_car_s>car_s) && (check_car_s - car_s < (ref_vel*ref_vel-check_speed*check_speed)/(2.24*2.24*5))){
//                 ref_vel = check_speed*2.24;
                	too_close = true;
                	if(lane>0 && left_clear){
                      lane -= 1;
                    }
                	else if(lane<2 && right_clear){
                      lane += 1;
                    }
              }
            }
            
            else if ((d>4*(lane-1)) && (d<4*lane)){
                double vx = sensor_fusion[i][3];
              	double vy = sensor_fusion[i][4];
              	double check_speed = sqrt(vx*vx+vy*vy);
              	double check_car_s = sensor_fusion[i][5];
              	check_car_s += ((double)prev_size*0.02*check_speed);
              	if( ((check_car_s>car_s) && (check_car_s - car_s < 30.0)) || ((check_car_s<car_s) && (double(j[1]["s"]) - check_car_s < 30.0))  ){
                  left_clear_check = false;
                }
            }
          ...
          left_clear = left_clear_check;
          right_clear = right_clear_check;
          if(too_close){
            ref_vel-=0.224;
          }
